    ...
    return {
      ...
      navigate: (url) => {
        if (typeof url === 'string') {
          ...
          const search = url.searchParams ? `?${url.searchParams.toString()}` : '';
  ...
  const session = React.useMemo(() => {
    ...
    return {
      user: {
        name: user.name || 'User',
        email: user.email || '',
        image: user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || 'User')}&background=2196f3&color=fff`,
      },
    };
  }, [user]);
